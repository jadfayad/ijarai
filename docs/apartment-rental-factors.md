...
| 3 | **Safety / Crime Rate** | **9** | Neighborhood safety is a hard filter, especially for families and women living alone. People check crime stats, lighting, general "feel" of the area at night. | Yes (`safety`, weight 9) |
...
| # | Factor | Score | Details | In App? |
|---|--------|-------|---------|---------|
| 6 | **Proximity to Public Transit** | **8** | Metro/bus stop within walking distance. Critical in cities where driving is impractical or expensive. Distinct from commute — captures optionality. | Yes (`transit`, weight 7) |
...
| 12 | **Proximity to Schools** | **7** | For families with children — school quality and distance is often the #1 factor, even above rent. For singles, irrelevant. Very persona-dependent. | Yes (`schools`, weight 5; curated per-city ratings) |
...
| 20 | **Proximity to Healthcare** | **5** | Hospital or clinic within 10–15 min. More important for elderly, families with young children, or people with chronic conditions. | Yes (`healthcare`, weight 7) |
...
| 29 | **Walkability Score** | **4** | General walk-score of the neighborhood. Correlates with amenities + transit but is its own lifestyle factor. | Yes (`walkability`, weight 8) |
| 30 | **Proximity to Green Space / Parks / Beach** | **4** | Park within 10-min walk. Strongly correlated with mental health and perceived livability. | Yes (`green_spaces`, weight 6) |
| 31 | **Utility Costs** | **3** | Electricity, water, district cooling (Dubai-specific), internet. Can add 10–25% on top of base rent. | Yes (`budget` has `include_utilities` toggle) |
| 35 | **Natural-hazard Exposure (flood / wildfire)** | **4** | Avoid flood plains and wildland-urban interface zones. Rising in importance post-2022 with climate-risk disclosure laws. | Yes (`hazard`, weight 4; curated per-city polygons) |
...
### Covered well
- Rent budget (with optional utility inclusion), commute (multi-destination), amenities, noise
- Neighborhood sub-scores as individual criteria: safety, walkability, green spaces, community, infrastructure, aesthetics, desirability
- Public transit proximity (distinct from commute)
- Healthcare access (distinct scoring: nearest hospital dominates, not density)
- Schools (curated per-city ratings with primary/secondary/all bands)
- Natural-hazard safety (curated per-city flood/wildfire polygons)

### Not covered but location-dependent and scoreable
- Parking availability
- Internet/fiber coverage
- Building density / views potential
- Crime-data integration (currently proxied by curated safety sub-score)
...
### Buyer-mode groundwork (data shipped, no UI yet)
- Property-tax rate per city (`property_tax.json`) — ready to wire into a future `mode: "buy"` path
...
- [x] **Safety / Crime Rate** (score 9) — Shipped as `safety` criterion (curated neighborhood sub-score). Upgrade path: integrate real crime incident data per cell.
- [x] **Public Transit Proximity** (score 8) — Shipped as `transit` criterion (Overpass stops + KDTree proximity/density).
- [ ] **Parking Availability** (score 7) — Score areas by street parking density, public parking lots, or zoning data
- [x] **Proximity to Schools** (score 7) — Shipped as `schools` criterion with per-city curated ratings by primary/secondary age band. Upgrade path: attendance-polygon overlays and live inspection-rating APIs.
- [ ] **Internet / Fiber Coverage** (score 6) — Integrate ISP coverage maps or fiber availability data
- [x] **Proximity to Healthcare** (score 5) — Shipped as `healthcare` criterion with emergency-access weighting (hospitals 2×, clinics 1×, pharmacies 0.5×).
- [x] **Walkability Score** (score 4) — Shipped as `walkability` criterion (curated neighborhood sub-score). Upgrade path: compute from street network + amenity density or integrate Walk Score API.
- [x] **Proximity to Green Space / Parks / Beach** (score 4) — Shipped as `green_spaces` criterion (curated neighborhood sub-score). Upgrade path: actual green-space area coverage metric.
- [ ] **Views / Aesthetics** (score 4) — Shipped as `aesthetics` sub-score; not yet data-driven by elevation/skyline. Upgrade path: integrate DEM + building-height data.
- [x] **Utility Costs** (score 3) — Shipped as `budget.include_utilities` toggle with per-city average (zone variation TODO).
- [x] **Natural-hazard Exposure** (score 4) — Shipped as `hazard` criterion (curated flood/wildfire polygons + STRtree). Upgrade path: FEMA/NOAA/Copernicus ingest.
